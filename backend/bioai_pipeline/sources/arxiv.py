...
class ArxivClient:
    ...
    def _parse_entry(self, entry: ET.Element, query_name: str) -> ArxivPaper:
        abstract_url = self._text(entry, "atom:id") or ""
        match = ARXIV_ID_RE.search(abstract_url)
        if not match:
            raise ArxivError(f"Unrecognized arXiv entry id: {abstract_url}")
        ...
        return ArxivPaper(
            ...
            abstract=self._text(entry, "atom:summary") or "",
